...
TowerResult = collections.namedtuple('TowerResult', ('inferred', 'target',
                                                     'prediction', 'grads'))
...
class Model(object):
  ...
  def _single_tower(self, tower_ind, feature):
    ...
    with tf.device('/gpu:%d' % tower_ind):
      with tf.name_scope('tower_%d' % (tower_ind)) as scope:
        ...
        losses, target, prediction = layers.evaluate(
            logits=inferred.logits,
            remakes=inferred.remakes,
            labels=feature['labels'],
            recons_image=feature['recons_image'],
    ...
    return TowerResult(inferred, target, prediction, grads)
  ...
  def _summarize_towers(self, targets, predictions, tower_grads):
  ...
  def multi_gpu(self, features, num_gpus):
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      for i in xrange(num_gpus):
        ...
        targets.append(tower_output.target)
        predictions.append(tower_output.prediction)
        tower_grads.append(tower_output.grads)

    summarized_results = self._summarize_towers(targets, predictions, tower_grads)
